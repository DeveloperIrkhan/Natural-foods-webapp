  ...
  const HomeTabBar = ({ selectedTab, onTabSelected }: IHomeTabBar) => {
    ...
    const { data: Categories, isSuccess: isCategoriesSuccess } =
      useGetCategoryQuery(undefined, {
        skip: storedCategories.length > 0
      });
    ...
    useEffect(() => {
      if (
        ...
        const latest = [...Products.products]
          ...
          .slice(0, 6);
        dispatch(setProductArray(latest));
      }
    ...
    const filteredProducts = useMemo(() => {
      ...
      if (selectedTab === "See all") return storedProducts;
      return storedProducts.filter((product) => product.category === selectedTab);
    }, [storedProducts, selectedTab]);
